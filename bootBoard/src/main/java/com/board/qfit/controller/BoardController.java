...
//	}
	...
	public String findAllPaging(@RequestParam(defaultValue = "1") int page, 
		...
		List<ChatRoomDTO> chatMessageList = chatRoomService.chatMessageList(page, size);
	...
	// 게시글 상세 + 댓글 조회
	@GetMapping
	public String findById(@RequestParam("boardno") Long boardno, Model model, HttpSession session) {
	...
	// 제목, 작성자, 내용 선택하여 검색
	@GetMapping("/search")
	public String search(@RequestParam(defaultValue = "1") int page, @RequestParam(defaultValue = "10") int size,
			@RequestParam String searchType, @RequestParam String keyword, Model model, HttpSession session) {
		String memberId = (String) session.getAttribute("memberId");
